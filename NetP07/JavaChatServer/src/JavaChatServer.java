	...
	class UserService extends Thread {
		...
		public UserService(Socket client_socket) {
			...
			try {
				...
				WriteAll(String.format("%s %s", UserName, "님께서 입장하셨습니다."));
			} catch (Exception e) {
		...
		public void run() {
			while (true) { // 사용자 접속을 계속해서 받기 위해 while문
				try {
					...
					String command = msg.split(" ")[1].toLowerCase(); // "/list" or "/exit" 등
					System.out.println("command : " + command);

					// list 구현 시작
					if(command.equals("/list")) {
						Iterator<UserService> it = UserVec.iterator();
						UserService us = null;
						int i=0;
						WriteOne("User List\nNo\tname\tUserStatus\n--------------------------");
						while(it.hasNext()) {
							us = it.next();
							WriteOne(String.format("%d\t%s\t%s\n", i, us.getUserName(), us.getUserStatus() ? "S":"O"));
							i++;
						}
						continue;
					}
					...
					// to 구현 시작
					// to java2 hi
					// msg -> [java1] /to java2 hi
					if(command.equals("/to")) {
						String toUser = msg.split(" ")[2]; // 귓속말 보낼 사용자
						String message = msg.substring(7+toUser.length()+this.UserName.length()); // 보낼 메시지
						System.out.println("user 글자수 : " + toUser.length());
						...
						while(it.hasNext()) {
							us = it.next();
							if(us.getUserName().equals(toUser) && us.getUserStatus() == false) {
								us.WriteOne(String.format("[귓속말] [%s] %s\n", us.getUserName(), message));
								break;
							}
						}
					} else {
					...
					if(command.equals("/sleep")) {
						...
						while(it.hasNext()) {
							...
							if(us.getUserName().equals(this.UserName)) {
								us.setStatus(true); // S로 바꾼다.
								break;
							}
							continue;
						}
						//System.out.println("현재 사용자 이름 " + this.UserName);
					}
					// sleep 구현 끝
